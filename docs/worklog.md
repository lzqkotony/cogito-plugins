...
## 2026-09-22 · 0.5.17（拟态 E.G.O.）

- 新增 `mimic` / `拟态` 套装与铁剑外形武器。
- 护甲按 y 生效：生命恢复、力量、四件套抗性提升 II。
- 实现拟态 I/II/III 状态机、致死保护、15 秒不可移动/攻击、15 分钟三阶段和模仿层数。
- 实现 20% 受伤或造成伤害叠层、5 层单体穿刺、10 层范围爆发和红色粒子。
- 增加阶段动态统一抗性：I=0.4、II=0.05、III=0.2。
- 首版材质采用红皮革 + 金色 `wayfinder` 纹饰，后续可按资源包替换。
- 版本 → **0.5.17**

## 2026-09-22 · 0.5.18（恢复失乐园原始伤害 <5 免疫）

- 恢复失乐园四件套的原始伤害免疫，判定在抗性和黄盾之前。
- 原始伤害严格 `<5` 时取消；`5` 点不会再被判定为免疫。
- 配置恢复 `damage-immunity-threshold: 5`，相关 Lore 和单元测试同步恢复。
- 版本 → **0.5.18**

## 2026-09-22 · 0.5.19（失乐园免伤阈值 5→10）

- 按原著护甲资料将失乐园四件套免伤从原始伤害 `<5` 调整为 `≤10`。
- 10 点及以下直接免疫，11 点起进入抗性、`x=0.1` 和黄盾结算。
- 更新配置、Lore 和单元测试。
- 版本 → **0.5.19**

## 2026-09-25 · 0.5.20（服主 E.G.O. NPC 保护）

- 修复服主 E.G.O. 误杀 Citizens NPC：识别实体上的 `NPC` metadata。
- 左键击杀、右键踢出和实体交互路径统一拦截 NPC。
- 保留普通玩家、生物和服主 E.G.O. 原保护逻辑。
- 版本 → **0.5.20**
