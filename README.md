...
- **失乐园 E.G.O.**：铁甲 + 红石 `wayfinder` 纹饰，按件数提供生命恢复、抗性提升、抗火与水下呼吸；四件套在乘 `x=0.1` 后最终伤害严格 `<5` 时免疫，并以每 `10s` 增长 1 点、最高 `20` 点的“神圣”黄盾抵挡伤害。铁锄空挥会攻击以玩家为中心、前后左右上下各 8 格内的生物，排除本人与召唤使徒；每 20 秒发动黑色痕迹特殊横扫，右键直接召唤 12 位使徒并令二者悬空 6 秒后落地
...
```bash
mvn clean package                       # 产物：target/Cogito-0.5.12.jar
...
| v0.5.12 | 失乐园改为 `±8` 立方体范围，排除本人与使徒；使徒不攻击召唤者，右键不再锄地 | ✅ 本版 |
| v0.5.x | 同一功能线的修复与小功能：E.G.O. 使用代价、共鸣、饰品四部位…… | 🚧 下一个 0.5.13 起 |
