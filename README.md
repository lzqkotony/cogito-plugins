...
- **失乐园 E.G.O.**：铁甲 + 红石 `wayfinder` 纹饰，按件数提供生命恢复、抗性提升、抗火与水下呼吸；四件套在全部防御计算前免疫原始伤害 `≤10`，其余伤害进入抗性和 `x=0.1` 计算，随后由每 `10s` 增长 1 点、最高 `20` 点的“神圣”黄盾抵挡。铁锄空挥会攻击以玩家为中心、前后左右上下各 8 格内的生物，排除本人与召唤使徒；每 20 秒发动黑色痕迹特殊横扫，右键直接召唤 12 位使徒并令二者悬空 6 秒后落地
...
```bash
mvn clean package                       # 产物：target/Cogito-0.5.20.jar
...
├── listener/EgoDamageListener.java      # E.G.O. 统一抗性、原始伤害 ≤10 免伤与神圣黄盾
...
| v0.5.17 | 新增拟态 E.G.O.：三阶段、模仿层数、右键单体/范围技能、红皮革金纹护甲 | ✅ 已发布 0.5.17 |
| v0.5.18 | 恢复失乐园四件套的“原始伤害 `<5` 免疫” | ✅ 已发布 0.5.18 |
| v0.5.19 | 按原著将失乐园免伤阈值提高为原始伤害 `≤10` | ✅ 已发布 0.5.19 |
| v0.5.20 | 服主 E.G.O. 不再击杀或踢出 Citizens NPC | ✅ 本版 |
| v0.5.x | 同一功能线的修复与小功能：E.G.O. 使用代价、共鸣、饰品四部位…… | 🚧 下一个 0.5.21 起 |
